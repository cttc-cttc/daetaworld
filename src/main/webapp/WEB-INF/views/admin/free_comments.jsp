																...
																<c:forEach var="list" items="${freeComments }">
																	<tr class="application-item">
																		<td class="application-job text-left">
																			<h3><a class="text-ellipsis" href="${contextPath }/board/free/detail?b_number=${list.b_number }#anc${list.c_number}">${list.c_contents }</a></h3>
																		</td>
	...
	<script>
		function deleteComments(c_num, m_id, r_type) {
			if(confirm('신고된 자유게시판 댓글을 삭제할까요?'))
				location.href = '${contextPath }/admin/warn_free_comments?c_num='+c_num +'&m_id='+m_id +'&r_type='+r_type;
		}
