...
<body class="template-color-1">
																...
																<c:forEach var="list" items="${tempingComments }">
																	<tr class="application-item">
																		...
																		<td class="view-application">
																			<a href="javascript:deleteComments(${list.c_number}, '${list.m_id }', '${list.r_type }')" class="view-application">댓글삭제</a>&nbsp;&nbsp;
															...
															<div class="col-12">
																<ul class="page-pagination justify-content-center">
																	...
																	<c:if test="${pp.getCurrPage() != 1 }">
																		<li><a href="${contextPath }/admin/temping_comments?page=${pp.getCurrPage()-1}"><i class="fa fa-angle-left"></i></a></li>
																	</c:if>
																	<c:if test="${pp.getCurrPage() == 1 }">
																	...
																	<c:forEach var="pageNaviNum" items="${pp.calcPageRange() }">
																		...
																		<c:if test="${pageNaviNum != pp.getCurrPage()}">
																			<li><a href="${contextPath }/admin/temping_comments?page=${pageNaviNum}">${pageNaviNum }</a></li>
																		</c:if>
																	</c:forEach>
																	<%-- 다음 버튼 활성/비활성 --%>
																	<c:if test="${pp.getCurrPage() != pp.calcLastPage() }">
																		<li><a href="${contextPath }/admin/temping_comments?page=${pp.getCurrPage()+1}"><i class="fa fa-angle-right"></i></a></li>
																	</c:if>
																	<c:if test="${pp.getCurrPage() == pp.calcLastPage() }">
	...
	<script>
		function deleteComments(c_num, m_id, r_type) {
			if(confirm('신고된 땜빵게시판 댓글을 삭제할까요?'))
				location.href = '${contextPath }/admin/warn_temping_comments?c_num='+c_num +'&m_id='+m_id +'&r_type='+r_type;
		}
		
		function cancelComments(cr_num) {
			if(confirm('신고된 땜빵게시판 댓글을 취소할까요?'))
				location.href = '${contextPath }/admin/cancel_temping_comments?cr_num='+cr_num;
		}
	</script>
</body>
</html>