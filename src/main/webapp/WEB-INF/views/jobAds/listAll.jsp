                  ...
                  <td>
					...
					<select id="selectOne" name="selectOne">
  						...
  						<c:forEach items="${listdrop}" var="row">
							<option value=" ">${row.a_name1}</option>
  						</c:forEach>
					</select>
						...
						<div class="table-responsive">
							<table class="table table-striped">
								...
								<c:forEach var="jobsend" items="${list }">
									<tr>
										<td><a href="listAllDetail?a_number=${jobsend.a_number}"> ${jobsend.a_number} </a></td>
										<td>${jobsend.a_date}</td>
										<td>${jobsend.a_time}</td>
										<td>${jobsend.a_wage}</td>
										<td>${jobsend.a_urgency }</td>
										<td>${jobsend.a_status }</td>
										<td>${jobsend.ua_date }</td>
										<td>${jobsend.a_type}</td>
...
$("#selectOne").change(function(){
	var select = $('#selectOne').val();//값 가져오기
	...
	$.ajax({
		...
		success: function(result){
			...
			gu += '<option value="' + result[i].Gu+ '">' + result[i].Gu + '</option>';
			}
			alert(gu);
			$('#Gu').html(gu);
		},
